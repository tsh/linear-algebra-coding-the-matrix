...
def vec_select_sum(veclist, k, D):
    ...
    True
    '''
    vecs_with_zero = vec_select(veclist, k)
    return vec_sum(vecs_with_zero, D)
...
def scale_vecs(vecdict):
    ...
    for k, v in vecdict.items():
        d = {}
        for vector_label in v.D:
            if vector_label in v.f:
                d[vector_label] = 1/k * v.f[vector_label]
        result.append(Vec(v.D, d))
    return result
